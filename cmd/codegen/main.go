...
func WriteUnmarshalFunction(name string, structType *ast.StructType, docs []*ast.Comment) (out string) {
	...
	for _, fd := range fields {
		...
		case "string":
			...
			if !ok {
				panic(fmt.Errorf("unable to parse WireLength for string field '%s' of struct '%s'", fd.name, name))
			}
			out += fmt.Sprintf("resp.%s, err = in.UTF16WithLengthPrefix(%d)\n", fd.name, length)
		case "VarByte":
	...
	for _, fd := range fields {
		...
		case "string":
			...
			if ok {
				...
				out += fmt.Sprintf("%s = string([]rune(%s)[:%d])\n", valToWrite, valToWrite, length)
				out += "}\n"
			}
